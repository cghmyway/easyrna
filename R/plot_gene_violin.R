...
#' @export
#'
#' @examples plot_gene_violin(data = data_tpm, groups = c("A","B"), group_positions = list(1:3,4:6), gene_of_interest = c("ENSGALG00000012172"), method = "t.test")
plot_gene_violin <- function(data, groups, group_positions, gene_of_interest, method = "t.test", colors = NULL) {
  ...
  # 绘制小提琴图，使用 log10(TPM + 1)
  p <- ggplot(combined_gene_data, aes(x = Group, y = log_TPM, fill = Group)) +
    geom_violin(trim = TRUE, color = "black") +  # 使用小提琴图，设置轮廓颜色
    theme_dose() +
